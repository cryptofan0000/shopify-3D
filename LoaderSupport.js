...
THREE.LoaderSupport.Callbacks.prototype = {
  ...
  setCallbackOnProgress: function (callbackOnProgress) {
    this.onProgress = THREE.LoaderSupport.Validator.verifyInput(
      callbackOnProgress,
      this.onProgress
    );
  },
  ...
  setCallbackOnReportError: function (callbackOnReportError) {
    this.onReportError = THREE.LoaderSupport.Validator.verifyInput(
  ...
  setCallbackOnMeshAlter: function (callbackOnMeshAlter) {
    this.onMeshAlter = THREE.LoaderSupport.Validator.verifyInput(
      callbackOnMeshAlter,
      this.onMeshAlter
    );
  },
  ...
  setCallbackOnLoad: function (callbackOnLoad) {
    this.onLoad = THREE.LoaderSupport.Validator.verifyInput(
      callbackOnLoad,
      this.onLoad
    );
  },
  ...
  setCallbackOnLoadMaterials: function (callbackOnLoadMaterials) {
    this.onLoadMaterials = THREE.LoaderSupport.Validator.verifyInput(
...
THREE.LoaderSupport.ResourceDescriptor = function (url, extension) {
  ...
  if (urlParts.length >= 2) {
    this.path = THREE.LoaderSupport.Validator.verifyInput(
  ...
  this.name = THREE.LoaderSupport.Validator.verifyInput(
    this.name,
    "Unnamed_Resource"
  );
  this.extension = THREE.LoaderSupport.Validator.verifyInput(
    extension,
    "default"
  );
...
THREE.LoaderSupport.ResourceDescriptor.prototype = {
  ...
  setResourcePath: function (resourcePath) {
    this.resourcePath = THREE.LoaderSupport.Validator.verifyInput(
      resourcePath,
      this.resourcePath
    );
  },
  ...
  checkResourceDescriptorFiles: function (resources, fileDesc) {
    ...
    for (var index in resources) {
      ...
      if (THREE.LoaderSupport.Validator.isValid(resource.content)) {
        ...
        if (!found)
          throw (
            'Unidentified resource "' + resource.name + '": ' + resource.url
          );
      } else {
        // fast-fail on bad type
        if (
...
THREE.LoaderSupport.MeshBuilder = function () {
  console.info(
    "Using THREE.LoaderSupport.MeshBuilder version: " +
      THREE.LoaderSupport.MeshBuilder.LOADER_MESH_BUILDER_VERSION
  );
...
THREE.LoaderSupport.WorkerSupport = function () {
  console.info(
    "Using THREE.LoaderSupport.WorkerSupport version: " +
      THREE.LoaderSupport.WorkerSupport.WORKER_SUPPORT_VERSION
  );
  this.logging = {
    enabled: true,
    debug: false,
  ...
  this.loaderWorker =
    typeof window !== "undefined"
      ? new THREE.LoaderSupport.WorkerSupport.LoaderWorker()
      : new THREE.LoaderSupport.WorkerSupport.NodeLoaderWorker();
};
...
THREE.LoaderSupport.WorkerSupport.prototype = {
  ...
  validate: function (
    ...
    userWorkerCode +=
      THREE.LoaderSupport.WorkerSupport.CodeSerializer.serializeClass(
        runnerImpl.runnerName,
        runnerImpl
      );
    ...
    var scope = this;
    if (
      THREE.LoaderSupport.Validator.isValid(libLocations) &&
...
THREE.LoaderSupport.WorkerSupport.LoaderWorker.prototype = {
  ...
  setCallbacks: function (meshBuilder, onLoad) {
    this.callbacks.meshBuilder = THREE.LoaderSupport.Validator.verifyInput(
      meshBuilder,
      this.callbacks.meshBuilder
    );
    this.callbacks.onLoad = THREE.LoaderSupport.Validator.verifyInput(
      onLoad,
  ...
  run: function (payload) {
    ...
    if (!THREE.LoaderSupport.Validator.isValid(this.callbacks.meshBuilder))
      throw 'Unable to run as no "MeshBuilder" callback is set.';
    if (!THREE.LoaderSupport.Validator.isValid(this.callbacks.onLoad))
  ...
  _postMessage: function () {
    if (
      THREE.LoaderSupport.Validator.isValid(this.queuedMessage) &&
      THREE.LoaderSupport.Validator.isValid(this.worker)
    ) {
...
THREE.LoaderSupport.WorkerRunnerRefImpl.runnerName =
...
THREE.LoaderSupport.NodeWorkerRunnerRefImpl.prototype = Object.create(
  THREE.LoaderSupport.WorkerRunnerRefImpl.prototype
);
THREE.LoaderSupport.NodeWorkerRunnerRefImpl.prototype.constructor =
  THREE.LoaderSupport.NodeWorkerRunnerRefImpl;
THREE.LoaderSupport.NodeWorkerRunnerRefImpl.runnerName =
  "THREE.LoaderSupport.NodeWorkerRunnerRefImpl";
...
THREE.LoaderSupport.WorkerSupport.NodeLoaderWorker.prototype = Object.create(
  THREE.LoaderSupport.WorkerSupport.LoaderWorker.prototype
);
THREE.LoaderSupport.WorkerSupport.NodeLoaderWorker.prototype.constructor =
  THREE.LoaderSupport.WorkerSupport.NodeLoaderWorker;
 ...
 * @inheritdoc
 */
THREE.LoaderSupport.WorkerSupport.NodeLoaderWorker.prototype.initWorker =
  function (code, runnerImplName) {
    var supportError = this.checkSupport();
    if (supportError) {
      throw supportError;
    }
...
THREE.LoaderSupport.WorkerDirector = function (classDef) {
  console.info(
    "Using THREE.LoaderSupport.WorkerDirector version: " +
      THREE.LoaderSupport.WorkerDirector.LOADER_WORKER_DIRECTOR_VERSION
  );
  this.logging = {
    enabled: true,
    debug: false,
...
THREE.LoaderSupport.WorkerDirector.prototype = {
  ...
  prepareWorkers: function (globalCallbacks, maxQueueSize, maxWebWorkers) {
    if (THREE.LoaderSupport.Validator.isValid(globalCallbacks))
      this.workerDescription.globalCallbacks = globalCallbacks;
    this.maxQueueSize = Math.min(
      maxQueueSize,
      THREE.LoaderSupport.WorkerDirector.MAX_QUEUE_SIZE
    );
    this.maxWebWorkers = Math.min(
      maxWebWorkers,
      THREE.LoaderSupport.WorkerDirector.MAX_WEB_WORKER
    );
  ...
  _buildLoader: function (instanceNo) {
    ...
    if (
      ...
      !THREE.LoaderSupport.Validator.isValid(loader.callbacks)
    ) {
      console.warn(
        classDef.name +
  ...
  _deregister: function (supportDesc) {
    if (THREE.LoaderSupport.Validator.isValid(supportDesc)) {
      ...
      if (THREE.LoaderSupport.Validator.isValid(loaderCallbacks.onProgress))
  ...
  tearDown: function (callbackOnFinishedProcessing) {
    ...
    this.instructionQueuePointer = this.instructionQueue.length;
    this.callbackOnFinishedProcessing =
      THREE.LoaderSupport.Validator.verifyInput(
