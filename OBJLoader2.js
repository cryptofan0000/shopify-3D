...
THREE.OBJLoader2 = function (manager) {
  console.info(
    "Using THREE.OBJLoader2 version: " + THREE.OBJLoader2.OBJLOADER2_VERSION
  );

  this.manager = THREE.LoaderSupport.Validator.verifyInput(
    manager,
...
THREE.OBJLoader2.prototype = {
  ...
  setModelName: function (modelName) {
    this.modelName = THREE.LoaderSupport.Validator.verifyInput(
      modelName,
      this.modelName
    );
  },
  ...
  setStreamMeshesTo: function (streamMeshesTo) {
    this.loaderRootNode = THREE.LoaderSupport.Validator.verifyInput(
      streamMeshesTo,
      this.loaderRootNode
    );
  },
  ...
  _setCallbacks: function (callbacks) {
    if (THREE.LoaderSupport.Validator.isValid(callbacks.onProgress))
      this.callbacks.setCallbackOnProgress(callbacks.onProgress);
    if (THREE.LoaderSupport.Validator.isValid(callbacks.onReportError))
      this.callbacks.setCallbackOnReportError(callbacks.onReportError);
    if (THREE.LoaderSupport.Validator.isValid(callbacks.onMeshAlter))
      this.callbacks.setCallbackOnMeshAlter(callbacks.onMeshAlter);
    if (THREE.LoaderSupport.Validator.isValid(callbacks.onLoad))
      this.callbacks.setCallbackOnLoad(callbacks.onLoad);
    if (THREE.LoaderSupport.Validator.isValid(callbacks.onLoadMaterials))
  ...
  onProgress: function (type, text, numericalValue) {
    ...
    if (THREE.LoaderSupport.Validator.isValid(this.callbacks.onProgress))
  ...
  _loadObj: function (
    ...
    // fast-fail
    if (!THREE.LoaderSupport.Validator.isValid(resource))
      onError(
    ...
    // fast-fail
    if (
      !THREE.LoaderSupport.Validator.isValid(resource.url) ||
      THREE.LoaderSupport.Validator.isValid(resource.content)
    ) {
      fileLoaderOnLoad(
        THREE.LoaderSupport.Validator.isValid(resource.content)
          ? resource.content
  ...
  _loadMtl: function (
    ...
    if (THREE.LoaderSupport.Validator.isValid(resource) && this.logging.enabled)
    ...
    var processMaterials = function (materialCreator) {
      ...
      if (
        THREE.LoaderSupport.Validator.isValid(resource) &&
        scope.logging.enabled
      )
    ...
    if (
      ...
      crossOrigin = THREE.LoaderSupport.Validator.verifyInput(
        crossOrigin,
        "anonymous"
      );
      mtlLoader.setCrossOrigin(crossOrigin);
      mtlLoader.setResourcePath(resource.resourcePath || resource.path);
      if (THREE.LoaderSupport.Validator.isValid(materialOptions))
...
THREE.OBJLoader2.Parser.prototype = {
  ...
  setMaterials: function (materials) {
    this.materials = THREE.LoaderSupport.Validator.verifyInput(
      materials,
      this.materials
    );
    this.materials = THREE.LoaderSupport.Validator.verifyInput(
  ...
  buildFace: function (faceIndexV, faceIndexU, faceIndexN) {
    ...
    if (this.useIndices) {
      ...
      var indicesPointer =
        this.rawMesh.subGroupInUse.indexMappings[mappingName];
  ...
  buildMesh: function (result) {
    ...
    for (var oodIndex in meshOutputGroups) {
      ...
      if (this.logging.enabled && this.logging.debug) {
        var materialIndexLine = THREE.LoaderSupport.Validator.isValid(
    ...
    this.callbackMeshBuilder(
      ...
      THREE.LoaderSupport.Validator.isValid(normalFA)
        ? [normalFA.buffer]
        : null,
